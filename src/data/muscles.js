...
import { state, getExtraSets, getLog, historyKey, getEffectiveProgram } from '../state/store.js';
import { getExerciseSwap } from '../state/store.js';

export const MUSCLE_GROUPS = [
  'Chest', 'Back', 'Quads', 'Hamstrings', 'Glutes',
  'Side Delts', 'Rear Delts', 'Front Delts',
  'Biceps', 'Triceps', 'Calves', 'Abs', 'Traps',
];
...
export function calcWeeklyVolume(currentWeek) {
  const volume = {};
  MUSCLE_GROUPS.forEach(g => { volume[g] = 0; });
...
export function calcDayVolume(dayIdx) {
  ...
  day.exercises.forEach((ex, e) => {
    ...
    const muscles = getMuscles(name);
    muscles.primary.forEach(g => { volume[g] = (volume[g] || 0) + ex.sets; });
    muscles.secondary.forEach(g => { volume[g] = (volume[g] || 0) + ex.sets * 0.5; });
  });
