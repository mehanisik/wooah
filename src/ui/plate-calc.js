...
function isBarbell(exerciseName) {
  const name = exerciseName.toLowerCase();
  const barbellTerms = ['barbell', 'bench press', 'squat', 'deadlift', 'ohp', 'row', 'hip thrust',
    'front squat', 'rdl', 'romanian', 'good morning', 'rack pull', 'floor press',
    'sumo', 'deficit', 'block pull', 'pendlay', 'curl (ez'];
...
export function calcPlates(targetWeight, exerciseName) {
  ...
  const available = getPlateSettings().plates.slice().sort((a, b) => b - a);
  const plates = [];
...
export function renderPlateBreakdown(weight, exerciseName) {
  ...
  Object.entries(grouped).sort((a, b) => b[0] - a[0]).forEach(([plate, count]) => {
