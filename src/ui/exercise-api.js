...
async function fetchAll() {
  ...
  try {
    ...
    const resp = await fetch(`${API_BASE}?limit=50&offset=0`, { signal: controller.signal });
    clearTimeout(timeout);
    if (!resp.ok) return [];
    const data = await resp.json();
    allExercises = data.data?.exercises || data.exercises || [];
    return allExercises;
  } catch {
    return [];
  }
...
export function preloadExerciseData() {
  fetchAll();
}
